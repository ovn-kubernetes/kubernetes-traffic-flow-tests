...
ENV_TFT_CAPABILITIES_POD = "TFT_CAPABILITIES_POD"
...
def get_tft_rdma_test_image() -> str:
    ...
    """
    manual_override = get_environ(ENV_TFT_RDMA_TEST_IMAGE)
    if manual_override:
        logger.info(f"env: {ENV_TFT_RDMA_TEST_IMAGE}={shlex.quote(manual_override)}")
        return manual_override
    # Auto-derive from base image
    base_image = get_environ(ENV_TFT_TEST_IMAGE) or ENV_TFT_TEST_IMAGE_DEFAULT
    rdma_image = _derive_rdma_image(base_image)
    logger.info(f"Auto-derived RDMA image: {shlex.quote(rdma_image)}")
    return rdma_image
...
@functools.cache
def get_tft_capabilities_pod() -> Optional[dict[str, list[str]]]:
    d = get_environ(ENV_TFT_CAPABILITIES_POD)
    if d is None:
        logger.info(f"env: {ENV_TFT_CAPABILITIES_POD}=")
        return None
    # Parse comma-separated list of capabilities (e.g., "NET_ADMIN,SYS_TIME")
    caps = [c.strip() for c in d.split(",") if c.strip()]
    value = {"add": caps}
    logger.info(f"env: {ENV_TFT_CAPABILITIES_POD}={d}")
    return value
