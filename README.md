...
Test cases 37-47 and 70-91 use OVN-Kubernetes User Defined Networks. The framework creates and cleans up a `{namespace}-udn` namespace with the appropriate UDN CRDs automatically. NetworkPolicies, MultiNetworkPolicies, and LoadBalancer services for UDN tests are also created in (and torn down from) the `{namespace}-udn` namespace.
