...
class ServerTask(Task, ABC):
    def __init__(self, ts: TestSettings):
        ...
        port_base = 5301 if pod_type == PodType.HOSTBACKED else 5201
        ...
        if connection_mode == ConnectionMode.EXTERNAL_IP:
            ...
            if connection_mode in _SECONDARY_MODES:
            ...
            if connection_mode in _SECONDARY_MODES:
                in_file_template = "host-pod-secondary-network.yaml.j2"
                pod_name = (
                    f"host-pod-secondary-network-{node_name_sanitized}-server-{port}"
                )
            else:
    ...
    def _create_setup_operation(
        ...
        if not provisioning:
            # Create MNPs per-test (never during global provisioning, because a
            # deny policy would block traffic for tests that run before this one)
    ...
    def __init__(self, ts: TestSettings, server: ServerTask):
        ...
        if pod_type == PodType.SRIOV:
            ...
            if connection_mode in _SECONDARY_MODES:
                in_file_template = "host-pod-secondary-network.yaml.j2"
                pod_name = (
                    f"host-pod-secondary-network-{node_name_sanitized}-client-{port}"
                )
            else:
                in_file_template = "host-pod.yaml.j2"
